...
def find_near_miss_story(sent_idx: int, all_sents: List[str], sentence_embeddings: List[np.ndarray]) -> str:
...
def main():
    ...
    # 1. Parse stories
    log_info("Parsing stories and sentences...")
    corpus_text = load_corpus(corpus_path)
    stories_raw = corpus_text.split("<|endoftext|>")
    
    stories_sents = []
    all_valid_sents = []
    
    for s_raw in stories_raw:
        s_raw = s_raw.strip()
        if not s_raw:
            continue
        sents = split_into_sentences(s_raw)
        sents = [s for s in sents if clean_and_tokenize(s)]
        if len(sents) >= 3:
            stories_sents.append(sents)
            all_valid_sents.extend(sents)
            
    log_info(f"Loaded {len(stories_sents)} stories containing {len(all_valid_sents)} sentences.")
    ...
    for story in stories_sents:
        for w_size in window_sizes:
            if len(story) < w_size:
                continue
                
            for i in range(len(story) - w_size + 1):
                pos_traj = story[i : i + w_size]
                neg_traj = list(pos_traj)
                
                neg_type = random.choice([
                    "shuffle_all", "swap_adjacent", "swap_middle", "swap_end",
                    "random_insert", "cross_story_mixing", "near_miss", "wrong_endings"
                ])
                ...
                elif neg_type == "cross_story_mixing":
                    # Replace last 2 sentences with sentences from a different story
                    other_story = random.choice([s for s in stories_sents if s != story])
                    for idx in range(w_size - 2, w_size):
                        neg_traj[idx] = random.choice(other_story)
                        
                elif neg_type == "wrong_endings":
                    # Replace the last sentence with a sentence from the beginning (relative index 0 or 1) of another story
                    other_story = random.choice([s for s in stories_sents if s != story])
                    neg_traj[-1] = other_story[0]
                    
                else: # "near_miss"
                    last_sent = pos_traj[-1]
                    s_idx = sent_to_idx.get(last_sent, random.randint(0, len(all_valid_sents)-1))
                    near_miss_sent = find_near_miss_story(s_idx, all_valid_sents, all_embs)
    ...
    for filename, split_data in [("train.json", train_data), ("val.json", val_data), ("test.json", test_data)]:
        ...
        log_info(f"Saved {len(split_data)} makes-sense samples to {out_path}")
