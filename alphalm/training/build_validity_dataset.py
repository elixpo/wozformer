...
def generate_phrase_swap(sent: str) -> str:
    ...
    # Select two random non-adjacent indices and swap
...
def generate_boundary_merge(sent_a: str, sent_b: str) -> str:
...
def mine_generated_failures(ablation_dir: Path, corpus_set: set) -> List[str]:
    failures = []
    ...
    for json_file in ablation_dir.glob("*.json"):
        try:
            ...
            log_info(f"  Warning: failed to parse generated text in {json_file}: {e}")
            
    failures = list(set(failures))
    log_info(f"Mined {len(failures)} unique generated failures from ablation logs.")
    return failures
...
def build_split(
    ...
    negatives = []
    # If training set, add mined failures
    if is_train:
        for f in mined_failures:
            negatives.append({"text": f, "label": 0.0, "type": "type6_mined"})
    ...
    if needed > 0:
        ...
        # Type 5: Merges without punctuation
        for _ in range(per_type):
            s1 = random.choice(corpus_sents)
            s2 = random.choice(corpus_sents)
            negatives.append({"text": generate_boundary_merge(s1, s2), "label": 0.0, "type": "type5_merge"})
...
def main():
    ...
    # Filter short or invalid sentences
    ...
    # Mine generated failures (Type 6)
    mined_failures = mine_generated_failures(ablation_dir, corpus_set)
    
    # Split corpus first to prevent data leakage!
    ...
    # Generate positive and negative samples for each split independently
