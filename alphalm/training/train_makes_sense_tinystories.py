...
from models.makes_sense_v2_1 import DeepMakesSenseBiGRUV2_1
from tokenizer import clean_and_tokenize
from embeddings import get_mean_vector
from utils import log_info, set_seed

class TrajectoryDatasetV2_1(Dataset):
    ...
    def _embed_trajectory(self, trajectory: list) -> np.ndarray:
        ...
        # Prefix Padding (insert zeros at the beginning) to prevent GRU decay
...
def train_makes_sense():
    ...
    log_info(f"Training TinyStories Makes-Sense v2.1 on device: {device}")
    ...
    train_dataset = TrajectoryDatasetV2_1(data_dir / "train.json", w2v)
    val_dataset = TrajectoryDatasetV2_1(data_dir / "val.json", w2v)
    
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False)
    
    # 3. Model
    model = DeepMakesSenseBiGRUV2_1(sentence_dim=w2v.vector_size, gru_hidden=128, dropout=0.2).to(device)
    ...
    best_model_path = ROOT_DIR / "models" / "makes_sense_tinystories.pt"
    ...
    if test_path.exists():
        test_dataset = TrajectoryDatasetV2_1(test_path, w2v)
        ...
        # We can construct a classification task where pos_scores are 1, neg_scores are 0
        y_true = [1.0] * len(y_pos_scores) + [0.0] * len(y_neg_scores)
        y_pred_scores = y_pos_scores + y_neg_scores
        ...
        with open(ROOT_DIR / "models" / "makes_sense_tinystories_metrics.json", "w") as f:
