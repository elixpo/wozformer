...
class IndexedValidityDataset(Dataset):
    def __init__(self, data_path: Path, w2v_model: Word2Vec, corpus_bigrams: set, max_len: int = 30):
        ...
        # Pre-convert to indices to speed up training epoch loops
...
def train_validity():
    ...
    log_info(f"Training Sentence Validity (BiGRU + Bigram Fraction) on device: {device}")
    ...
    epochs = 15 # Converges extremely fast now
    best_val_acc = 0.0
    best_model_path = ROOT_DIR / "models" / "sentence_validity.pt"
    ...
    if test_path.exists():
        test_dataset = IndexedValidityDataset(test_path, w2v, corpus_bigrams)
