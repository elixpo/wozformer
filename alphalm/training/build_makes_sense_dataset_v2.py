...
def find_near_miss(sent_idx: int, corpus_sents: List[str], sentence_embeddings: List[np.ndarray]) -> str:
    ...
    for idx, emb in enumerate(sentence_embeddings):
        # Exclude contiguous sentences within window of 10 to ensure E is from elsewhere
    ...
    # Pick a near-miss from the top 10 most semantically similar sentences
...
def generate_trajectory_pairs(
    ...
    for w_size in window_sizes:
        for domain_name, sents, embs, other_sents in domains:
            # Limit samples per window size to avoid giant datasets
            limit = 1000
            count = 0
            for i in range(len(sents) - w_size + 1):
                ...
                # Choose a negative corruption type
    ...
    # 2. Mine Search-log failures (Type H)
    log_info("Mining search logs from ablation_results...")
    mined_count = 0
    if ablation_dir.exists():
        for json_file in ablation_dir.glob("*.json"):
            try:
                ...
                # Skip very short or empty paths
                if len(path_indices) < 3:
                    continue
                    
                # Weight proportional to path score (relative value weight)
                weight = max(1.0, float(path_score) / 10.0)
                
                # Reconstruct text sentences from indices
                # Note: Ablation logs ran on sales corpus
                run_sents = [sales_sents[idx] for idx in path_indices]
                
                for step_idx, step_log in enumerate(decision_logs):
                    # We can construct window context of size up to 5 sentences + 1 candidate
                    step_num = step_log["step"] # 1-indexed
                    if step_num < 2:
                        continue
                    ...
                    for alt in rejected_candidates[:2]:
                        ...
                        # Pad or slice trajectories to standard length (between 3 and 6 sentences)
                        # Keep it to the exact length of pos_traj
                        pairs.append({
                ...
                log_info(f"  Warning: failed to parse search log {json_file}: {e}")
...
def main():
    ...
    ablation_dir = ROOT_DIR / "ablation_results"
    ...
    # Load aligned Word2Vec
    w2v_path = ROOT_DIR / "evaluator" / "evaluator_w2v.model"
    w2v = Word2Vec.load(str(w2v_path))
    
    # 2. Build Dataset Pairs
    pairs = generate_trajectory_pairs(sales_sents, newton_sents, w2v, ablation_dir)
    ...
    # Save processed data
    data_dir = ROOT_DIR / "models" / "makes_sense_v2_data"
