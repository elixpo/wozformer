...
def train_validity():
    ...
    log_info(f"Training TinyStories Sentence Validity (BiGRU + 7 Features) on device: {device}")
    ...
    epochs = 15
    best_val_acc = 0.0
    best_model_path = ROOT_DIR / "models" / "validity_tinystories.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for batch_x, batch_f, batch_y in val_loader:
                ...
                all_val_preds.extend(preds.view(-1).cpu().numpy().tolist())
                all_val_labels.extend(batch_y.view(-1).cpu().numpy().tolist())
    ...
    if test_path.exists():
        ...
        # Calculate precision, recall, f1 manually to avoid scikit-learn dependency
        ...
        # Let's write a simple ranking based AUC
        ...
        # Save metrics dictionary as metadata JSON
        metrics_dict = {
            "accuracy": test_acc,
            "precision": precision,
            "recall": recall,
        ...
        with open(ROOT_DIR / "models" / "validity_tinystories_metrics.json", "w") as f:
