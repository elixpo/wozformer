...
class LateConcatValidityBiGRU(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fraction: torch.Tensor) -> torch.Tensor:
        ...
        # Binary indicator: 1.0 if fraction >= 0.999 else 0.0
        is_perfect = (fraction >= 0.999).float()
        
        combined = torch.cat((gru_feats, fraction, is_perfect), dim=1)
...
def train_and_eval(freeze_emb: bool):
    ...
    print(f"\nTraining late-concat binary-indicator validity (freeze_emb={freeze_emb}) on device: {device}")
