...
from models.sentence_validity import SentenceValidityBiGRU, SentenceValidityEvaluator
from search import AlphaLMSearcher
from tokenizer import clean_and_tokenize

class TestSentenceValidity(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.test_dir = ROOT_DIR / "validity_test_temp"
        ...
        cls.model = SentenceValidityBiGRU(
            vocab_size=cls.vocab_size,
            word_dim=100,
            gru_hidden=64,
            dropout=0.0
        )
        cls.checkpoint_path = cls.test_dir / "test_sentence_validity.pt"
    ...
    def test_model_forward(self):
        """Test validity model forward pass and output bounds."""
        dummy_x = torch.randint(0, self.vocab_size, (5, 30))
        dummy_f = torch.rand(5, 1)
    ...
    def test_evaluator_inference(self):
        """Test validity evaluator wrapper initialization and scoring."""
        evaluator = SentenceValidityEvaluator(
    ...
    def test_search_integration_with_validity(self):
        """Test beam search integration when sentence validity head is active."""
        evaluator = SentenceValidityEvaluator(
            model_path=self.checkpoint_path,
            w2v_path=self.w2v_path
        )
