...
class _FeedForward(nn.Module):
    def __init__(self, embed_dim: int, mult: int = 2):
        super().__init__()
...
class _Block(nn.Module):
    def __init__(self, embed_dim: int, num_heads: int, block_size: int, mult: int):
        super().__init__()
        self.ln1 = nn.LayerNorm(embed_dim)
        self.attn = _MultiHead(embed_dim, num_heads, block_size)
        self.ln2 = nn.LayerNorm(embed_dim)
        self.mlp = _FeedForward(embed_dim, mult=mult)
...
class TinyTransformer(nn.Module):
    ...
    def __init__(self, cfg: TransformerConfig, block_size: int):
        ...
        self.blocks = nn.Sequential(
            *[
                _Block(cfg.d_model, cfg.num_heads, block_size, cfg.mlp_mult)
