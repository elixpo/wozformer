...
class TinyStoriesPolicyDataset(Dataset):
    def __init__(self, json_path: Path, w2v_model_path: Path, window_size: int = 4):
        ...
        log_info(f"Embedding {len(data)} policy samples from {json_path.name}...")
        for item in data:
...
def train_policy():
    ...
    log_info(f"Training TinyStories Policy Head on device: {device}")
    
    w2v_path = ROOT_DIR / "models" / "tinystories_word2vec.model"
    data_dir = ROOT_DIR / "policy" / "data_tinystories"
    
    train_dataset = TinyStoriesPolicyDataset(data_dir / "train.json", w2v_path)
    val_dataset = TinyStoriesPolicyDataset(data_dir / "val.json", w2v_path)
    
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False)
    
    input_dim = 4 * 128 # window_size * embedding_dim = 4 * 128 = 512
    ...
    patience_counter = 0
    epochs = 20
    checkpoint_path = ROOT_DIR / "models" / "policy_tinystories.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            log_info(f"  --> Saved new best checkpoint to {checkpoint_path.name}")
        else:
            patience_counter += 1
    ...
    if test_path.exists():
        test_dataset = TinyStoriesPolicyDataset(test_path, w2v_path)
        ...
        with open(ROOT_DIR / "models" / "policy_tinystories_metrics.json", "w") as f:
