...
larger than nb12c and is not itself a deployment candidate.
...
$V{=}256$ BPE tokeniser except nb12 (F6).}
\label{tab:main}
\small
...
\textbf{HDC-RWKV $V{=}256, d{=}256$ (nb12c)} & 131{,}584   & \textbf{16.4~KB} & \textbf{4.35} & \textbf{2.93} & \textbf{135~K} \\
Teacher (fp32 transformer)      & 3{,}000{,}000            & 12~MB fp32 & 3.55 & 2.03 & --- \\
\bottomrule
\end{tabular}
\end{table}

The shipping HDC-RWKV improves BPC over the equal-budget nb12 variant
...
nb12c ($2.93$). Deployment cost rises from 16~KB to 148~KB (a
...
bipolar decay in nb12c is therefore not an imposed constraint; it is
...
(nb18). All three distilled runs regress against the NLL-only
...
gap between the shipping model (nb12c, BPC 2.93) and the distilled
...
Source, notebooks, and checkpoints are available at the project
repository (URL redacted for review). Appendix~\ref{app:walkthrough}
provides a hand-verifiable numerical trace on a toy instance.
...
The companion document \texttt{docs/figures/architecture\_walkthrough.tex}
provides a complete numerical trace of a training step (forward, loss,
backward through the Straight-Through Estimator) and an inference step
(greedy and top-$k$ sampling) on a hand-verifiable toy instance of the
shipping architecture with $V{=}4$, $d{=}4$, $T{=}3$, $L{=}1$. That
document is designed for the reader who wants to reproduce every number
in Equations~\ref{eq:embed}--\ref{eq:logits} by mental arithmetic.

\begin{figure}[h]
\centering
\fbox{\parbox{0.85\linewidth}{\centering \small
\vspace{4mm}
\emph{[Figure: expanded HDC-RWKV architecture poster --- will be
included from \texttt{docs/figures/architecture\_hdc\_rwkv\_expanded.pdf}
in the final build via \texttt{\textbackslash includegraphics}. This
poster opens every block from Figure~\ref{fig:archcompact} to show the
cell-level operations: the vocabulary embedding table, the cyclic
right-rotation acting on individual channels, the per-channel
recurrence wiring, the stack of prototype dot products, and the STE
forward/backward and XOR/popcount deployment equivalents.]}
\vspace{4mm}
}}
\caption{Expanded HDC-RWKV architecture poster. Each of the four
blocks in Figure~\ref{fig:archcompact} is opened up to show its
internal cell-level operations, with auxiliary panels for the
Straight-Through Estimator and the XOR/popcount deployment
equivalent.}
