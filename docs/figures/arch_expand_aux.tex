...
% ========== AUX A: STE ==========
\node[block, minimum height=54mm, anchor=north west] (AA) at (0,0) {};
...
\begin{scope}[shift={(3.5,-3.0)}]
  \node[subHead] at (-2.2, 1.4) {Forward: $y = \mathrm{sign}(x)$};
...
\begin{scope}[shift={(11.5,-3.0)}]
  \node[subHead] at (-2.2, 1.4) {Backward: $\partial y/\partial x = \mathbb{1}[|x|\le 1]$};
  \draw[->] (-2.0,0) -- (2.0,0) node[right, font=\scriptsize] {$x$};
...
% ========== AUX B: XOR/POPCOUNT ==========
\node[block, minimum height=58mm, anchor=north west] (AB) at (0,-60mm) {};
...
\begin{scope}[shift={(1.5,-7.4)}]
  ...
  \begin{scope}[shift={(0.4,-2.1)}]
    ...
    \node[font=\small, anchor=west, align=left] at (8*0.9 + 1.0, -0.8)
      {popcount $= 4$\\
       $d - 2\cdot 4 = 0$\\
       \emph{(orthogonal)}};
  \end{scope}
