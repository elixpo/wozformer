% Blocks 1 + 2 of the HDC-RWKV expansion — token lookup + position binding.
...
\begin{scope}[shift={(5.5,-1.5)}]
  ...
  \node[idx, anchor=east] at (-0.3, -3.4) {$\vdots$};
  \node[idx] at (3*1.1, -3.4) {$\vdots$};

  \draw[decorate, decoration={brace, amplitude=5pt}]
    (8*1.1 + 0.65, -0.35) -- (8*1.1 + 0.65, -3.15);
  \node[matlbl, anchor=west] at (8*1.1 + 1.1, -1.7)
...
\node[block, minimum height=52mm, anchor=north west] (B2) at (0,-58mm) {};
...
\begin{scope}[shift={(5.5,-7.7)}]
  ...
  \node[lbl, anchor=east, align=right] at (-0.3, -1.65)
  ...
  \node[matlbl, align=center, anchor=west, text width=42mm]
     at (8.3, -0.6)
    {$\rho^{t}(\bh)_j = \bh_{(j-t)\bmod d}$\\[2pt]
     \emph{parameter-free,}\\
     \emph{preserves $\bipset^{d}$}};
\end{scope}
