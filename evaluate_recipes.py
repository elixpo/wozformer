...
class ProceduralConsistencyScorer:
    ...
    def score_path(self, sentences: List[str]) -> float:
        ...
        # Split target generation of length 8 into:
        # First 2 sentences (25%): Early steps (prepare/mix) -> relative position <= 0.35
        # Middle 4 sentences (50%): Cooking steps (bake/boil/stir) -> 0.20 <= relative position <= 0.80
        # Last 2 sentences (25%): Final steps (cool/serve/plate) -> relative position >= 0.65
        beg_correct = 0
        beg_total = 2
        dev_correct = 0
        dev_total = 4
...
def main():
    ...
    pol_path = ROOT_DIR / "models" / "policy_recipes.pt"
    ...
    log_info("Initializing searcher and evaluators (ordered for fast caching)...")
    ms_eval = DeepMakesSenseEvaluatorTransformer(model_path=ms_path, w2v_path=w2v_path)
    
    # 1. Initialize searcher first to batch tokenize all sentences and warm up _TOKEN_CACHE
    ...
    # 2. Initialize validity evaluator second (passes valid_sents as list; hits cache instantly)
    ...
    # 3. Initialize consistency scorer third (hits cache instantly during token filtering)
    ...
    configs = {
        ...
        "B_policy_only": {
            ...
            "weights": {
                ...
                "global": 3.0,
                "completion": 0.0,
                "makes_sense": 0.0,  # MS disabled
                "policy": 1.0,
                "validity": 0.0,  # Validity disabled
                "sentence_rep": 1.0,
                "semantic_rep": 0.25,
                "topic_rep": 2.25,
                "topic_progress": 0.5,
        ...
        "C_makes_sense_only": {
            "name": "Configuration C (Transformer Makes-Sense Only B=5)",
            "beam_width": 5,
            "weights": {
                "boundary": 1.0,
                "local": 3.0,
    ...
    for cid, conf in configs.items():
        ...
        # Configure searcher active evaluators dynamically for this run
    ...
    print("\n\n=============================================================")
    print("                     ALPHA-LM V6.5.1 RECIPE ABLATION SUMMARY")
    print("=============================================================")
    ...
    for cid, res in ablation_results.items():
        print(f"| {res['name'][:35]}... | {res['avg_score']:.2f} | {res['avg_ms']:.3f} | {res['avg_val']:.3f} | {res['avg_repetition']*100:.1f}% | {res['avg_diversity']:.3f} | {res['avg_progress']:.3f} | {res['avg_consistency']*100:.1f}% | {res['avg_runtime']:.3f} |")
        
    # Generate reports/recipe_generation_samples.md
    reports_dir = ROOT_DIR / "reports"
    reports_dir.mkdir(parents=True, exist_ok=True)
    samples_file = reports_dir / "recipe_generation_samples.md"
    
    with open(samples_file, "w", encoding="utf-8") as f:
        f.write("# AlphaLM v6.5.1 Qualitative Recipe Generation Samples\n\n")
        for seed in seeds:
    ...
    report_file = reports_dir / "alphaLM_v6_5_1_recipe_report.md"
    with open(report_file, "w", encoding="utf-8") as f:
        f.write("# AlphaLM v6.5.1 Recipe Trajectory Learning Experiment Report\n\n")
        ...
        # Placeholders for research questions to be answered in final step
        f.write("\n## Research Questions & Answers\n\n")
        f.write("### Q1: Does AlphaLM learn procedural planning better than narrative planning?\n")
        f.write("[To be answered in report phase]\n\n")
        f.write("### Q2: Does the Transformer Makes-Sense evaluator outperform BiGRU on procedural trajectories?\n")
        f.write("[To be answered in report phase]\n\n")
        f.write("### Q3: Do recipe trajectories exhibit stronger long-range structure than TinyStories?\n")
        f.write("[To be answered in report phase]\n\n")
        f.write("### Q4: Does the Full Ensemble achieve higher Procedure Consistency than TinyStories Narrative Consistency?\n")
        f.write("[To be answered in report phase]\n\n")
        f.write("### Q5: What kinds of procedural dependencies were learned?\n")
        f.write("[To be answered in report phase]\n\n")
