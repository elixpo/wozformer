...
def main():
    ...
    log_info(f"Loaded {len(valid_sents)} valid sentences from recipe subset.")
    ...
    for filename, split_data in [("train.json", train_data), ("val.json", val_data), ("test.json", test_data)]:
        ...
        log_info(f"Saved {len(split_data)} recipe validity samples to {out_path}")
