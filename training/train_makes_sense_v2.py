...
from models.makes_sense_v2 import DeepMakesSenseBiGRU
...
class TrajectoryDataset(Dataset):
    ...
    def _embed_trajectory(self, trajectory: list) -> np.ndarray:
        ...
        while len(embs) < self.max_len:
            embs.append(np.zeros(self.vector_size, dtype=np.float32))
...
def train_model():
    ...
    log_info(f"Training Makes-Sense v2 on device: {device}")
    ...
    train_dataset = TrajectoryDataset(data_dir / "train.json", w2v)
    val_dataset = TrajectoryDataset(data_dir / "val.json", w2v)
    
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False)

    # Initialize model
    model = DeepMakesSenseBiGRU(sentence_dim=w2v.vector_size, gru_hidden=128, dropout=0.2).to(device)
    
    # Margin Ranking Loss
    # We want pos_score > neg_score + margin, so target is 1
    margin = 0.1
    ...
    best_model_path = ROOT_DIR / "models" / "makes_sense_v2.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        for pos_batch, neg_batch, weight_batch in train_loader:
            ...
            # Forward pass
            pos_scores = model(pos_batch).squeeze(-1) # [batch_size]
            neg_scores = model(neg_batch).squeeze(-1) # [batch_size]
            
            # Target = 1 for MarginRankingLoss (representing that pos_scores should be ranked higher than neg_scores)
            target = torch.ones_like(pos_scores).to(device)
            
            # Compute unreduced margin ranking loss
            loss = criterion(pos_scores, neg_scores, target) # [batch_size]
            
            # Apply trajectory weight
            ...
            # Accuracy metric: how often is positive scored higher than negative
    ...
    if test_path.exists():
        test_dataset = TrajectoryDataset(test_path, w2v)
        ...
        log_info(f"Final Test Ranking Accuracy: {test_acc*100:.2f}%")
