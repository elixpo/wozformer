...
class DeepMakesSenseBiGRUV2_1(nn.Module):
    def __init__(self, sentence_dim: int = 100, gru_hidden: int = 128, dropout: float = 0.2):
        """
        AlphaLM Makes-Sense v2.1 Trajectory Evaluator.
        Uses a Bidirectional GRU to model sentence order in a trajectory,
        incorporating Layer Normalization and Dropout for robust generalization.
        """
        super().__init__()
        self.gru = nn.GRU(
        ...
        # Layer Normalization for stability and robust feature representation
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        traj_emb = self.layer_norm(traj_emb)
...
class DeepMakesSenseEvaluatorV2_1:
    def __init__(self, model_path: Path = None, w2v_path: Path = None):
        """
        Wrapper class for Deep Makes-Sense v2.1 trajectory evaluator.
        """
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model_path = model_path or getattr(root_config, "MAKES_SENSE_V2_1_PATH", root_config.BASE_DIR / "models" / "makes_sense_v2_1.pt")
        ...
        # Initialize model
        self.model = DeepMakesSenseBiGRUV2_1(
